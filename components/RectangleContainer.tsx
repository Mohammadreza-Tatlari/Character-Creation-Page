...
export default function RectangleContainerDynamic() {

  let ArrayOfContent = [1,2,3,4,5,6,1,2,3,4,5,6,1,2,3,4,5,6,1,2,3,4,5,6,1,2,3,4,5,6,1,2,3,4,5,1];
  // rounded-tl-lg rounded-tr-none rounded-b-none
  return (
    <div className="w-[337px] h-[325px] shrink-0 flex flex-row items-center justify-end gap-[0px_9px]">
      <div className="grid grid-cols-6 items-center justify-start gap-[1px_1px] overflow-hidden shrink-0">
       {
        ArrayOfContent.map((items , index) => (
          ...
          w-[55px] relative bg-gray-400 h-[55px]`} />
        ))
       }
      </div>
      <div className="w-[3px] relative rounded-4xl bg-gray-600 h-[323px]">
        <div className=" rounded-4xl bg-lawngreen-100 w-[3px] h-[42px]" />
      </div>
    </div>
  );
};
